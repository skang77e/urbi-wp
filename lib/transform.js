...
const paletteTransformer = (node, doc, map) => {
  ...
  const keys = ['urbi', 'palette', ...key.split('.')];

  // @ts-ignore
  const value = doc.getIn(keys);
  if (value === undefined) {
    throw map.errorForRange(
      `Could not resolve '${key}' in urbi.palette`,
      node.range,
    );
  }
...
const colorTransformer = (node, doc, map) => {
  ...
  const keys = ['urbi', 'colors', ...key.split('.')];

  // Start by checking if the value is a reference to urbi.colors.
  // @ts-ignore
...
const fontFamilyTransformer = (node, doc, map) => {
  ...
  const key = String(nodeValue);
  const keys = ['urbi', 'typography', 'font-family', ...key.split('.')];

  // @ts-ignore
  const value = doc.getIn(keys, /* keep node */ true);
  if (value === undefined) {
    throw map.errorForRange(
      `Could not resolve '${key}' in urbi.colors`,
      node.range,
    );
  }
...
function getScalarVisitor(path) {
  // You are here: ['urbi', 'colors', ...]
  if (path.length > 2 && path[0] === 'urbi' && path[1] === 'colors') {
    return paletteTransformer;
  }

  // NB. You are here: ['urbi', 'typography', 'display', *, key]
  if (
    path.length === 5 &&
    path[0] === 'urbi' &&
    path[1] === 'typography' &&
    ...
    return (node, doc, map) => {
      ...
      const key = nodeValue;
      const keys = ['urbi', 'typography', prefix];

      // First, attempt a lookup of the typography at urbi.typography - this'll weed
